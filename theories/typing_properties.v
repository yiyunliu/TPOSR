...
Lemma wt_unique : forall Γ a b c A, Γ ⊢ a ▻ b ∈ A -> forall B, Γ ⊢ a ▻ c ∈ B -> Γ ⊢ A ≡ B.
  hauto lq:on use:unique_mutual, lh_refl_mutual. Qed.
...
Definition ηmorphing2_ok ρ0 ρ1 Γ Δ :=
  forall n A, lookup n Γ A -> Δ ⊢ ρ0 n ▻η ρ1 n ∈ A[ρ0].

Lemma ηmorphing2_ren ρ0 ρ1 ξ Γ Δ Ξ :
  ηmorphing2_ok ρ0 ρ1 Γ Δ ->
  lookup_good_renaming ξ Δ Ξ ->
  ⊢ Ξ ->
  ηmorphing2_ok (funcomp (ren_tm ξ) ρ0) (funcomp (ren_tm ξ) ρ1) Γ Ξ.
Proof.
  rewrite /ηmorphing2_ok. move => hρ hξ hΞ n A hn.
  rewrite /funcomp. asimpl.
  have -> : A [funcomp (ren_tm ξ) ρ0] = A[ρ0]⟨ξ⟩ by asimpl.
  apply : wt_η_renaming; eauto.
Qed.

Lemma ηmorphing2_ext ρ0 ρ1 Γ Δ a0 a1 A  :
  ηmorphing2_ok ρ0 ρ1 Γ Δ ->
  Δ ⊢ a0 ▻η a1 ∈ A[ρ0] ->
  ηmorphing2_ok (scons a0 ρ0) (scons a1 ρ1) (A :: Γ) Δ.
Proof.
  move => hρ ha.
  move => n A0.
  elim /lookup_inv=>//=_.
...
Lemma ηmorphing2_ok_embed ρ0 ρ1 Γ Δ :
  ηmorphing2_ok ρ0 ρ1 Γ Δ -> lookup_good_morphing2 ρ0 ρ1 Γ Δ.
Proof. sfirstorder use:WtExp_embed. Qed.

Lemma ηmorphing2_ok_embed' ρ0 ρ1 Γ Δ :
  ηmorphing2_ok ρ0 ρ1 Γ Δ -> lookup_good_morphing ρ0 Γ Δ.
Proof.
  hauto lq:on use:ηmorphing2_ok_embed, lookup_good_morphing2_lh_refl.
Qed.

Lemma ηmorphing2_up ρ0 ρ1 Γ Δ A i :
  ηmorphing2_ok ρ0 ρ1 Γ Δ ->
  Δ ⊢ A [ρ0] ∈ Univ i ->
  ηmorphing2_ok (up_tm_tm ρ0) (up_tm_tm ρ1) (A :: Γ) (A[ρ0] :: Δ).
Proof.
  move => h0 h1. asimpl.
  apply ηmorphing2_ext. apply : ηmorphing2_ren; eauto with wt.
  apply lookup_good_renaming_shift.
  apply : WE_Var; eauto with wt. apply here'. by asimpl.
Qed.


Lemma ηmorphing2 Γ a b A ρ0 ρ1 Δ : Γ ⊢ a ▻η b ∈ A ->
  ηmorphing2_ok ρ0 ρ1 Γ Δ -> Wf Δ -> Δ ⊢ a[ρ0] ▻η b[ρ1] ∈ A[ρ0].
Proof.
  move => h. move : Δ ρ0 ρ1. elim : Γ a b A /h; try solve [simpl in *; eauto with eexp].
  - hauto lq:on use:wr_lh_refl, WtExp_embed, ηmorphing2_up db:wt,eexp.
  - move => Γ A A' i B M M' hA ihA hB hM ihM Δ ρ0 ρ1 hρ hΔ /=.
    have hρ0 : lookup_good_morphing ρ0 Γ Δ by hauto l:on use:ηmorphing2_ok_embed'.
    have hA' : Δ ⊢ A[ρ0] ∈ Univ i by hauto lq:on use:wt_morphing_univ, WtExp_embed, wr_lh_refl.
    have : ηmorphing2_ok (up_tm_tm ρ0) (up_tm_tm ρ1) (A :: Γ) (A [ρ0] :: Δ)
      by apply : ηmorphing2_up; eauto.
    move => /[dup] hρ' /ηmorphing2_ok_embed' hρ''.
    apply : WE_Lam; eauto with bred.
    apply : wt_morphing_univ; eauto using ηmorphing2_ok_embed with wt.
    eauto with wt.
  - move => Γ A i B B' M M' N N' hA hB ihB hM ihM hN ihN Δ ρ0 ρ1 hρ hΔ /=.
...
Module OExp.
  ...
  Lemma ηcommutativity Γ a b c A :
    Γ ⊢ a ▻η b ∈ A ->
    R Γ a c A ->
  ...
  Lemma βcommutativity Γ a b c A :
    Γ ⊢ a ▻β b ∈ A ->
    R Γ a c A ->
    exists d, R Γ b d A /\ Γ ⊢ c ▻β d ∈ A.
  Proof.
    move => h. elim /inv => //=_.
    move => a0 A0 i B U hA0 hB ha e *. subst.
    have ? : ⊢ A0 :: Γ by  sfirstorder use:Wt_Wf_mutual.
    eexists. split. econstructor; eauto.
    apply : WR_Conv'; eauto.
    hauto lq:on use:WtBRed_embed, wr_rh_refl.
    econstructor; eauto.
    econstructor; eauto using β_lh_refl with bred.
    have e0 : B = subst_tm (scons (var_tm var_zero) var_tm) (ren_tm (upRen_tm_tm shift) B)by asimpl; rewrite subst_id.
    rewrite {3}e0 => {e0}.
    apply : WB_App. apply : wt_renaming_univ. apply hA0. apply lookup_good_renaming_shift.
    sfirstorder use:Wt_Wf_mutual.
    change (Univ i) with (ren_tm (upRen_tm_tm shift) (Univ i)). apply : wt_β_renaming; eauto with bred.
    sfirstorder use:β_lh_refl.
    apply good_renaming_up. apply lookup_good_renaming_shift.
    econstructor. apply : wt_renaming_univ; eauto. apply lookup_good_renaming_shift.
    change (Pi A0 ⟨ shift ⟩ B ⟨ upRen_tm_tm shift ⟩) with (ren_tm shift (Pi A0 B)).
    apply : wt_β_renaming; eauto using Equiv_sym with bred.
    apply lookup_good_renaming_shift.
    constructor=>//. constructor.
  Qed.
...
Module OExps.
  ...
  Proof.
    ...
    - hauto lq:on ctrs:R use:OExp.ηcommutativity.
  Qed.
  ...
  Proof.
    ...
    elim : a c A /h.
    - move => a A ha b hb. exists b.
      split. apply Refl. sfirstorder use:wr_rh_refl, WtBRed_embed.
      qauto l:on use:wr_rh_refl.
    - hauto lq:on ctrs:R use:OExp.βcommutativity.
  Qed.
...
Proof.
  ...
  - move => Γ i A A' B B' hA ihA hB ihB T U.
    ...
    move :  OExps.ηcommutativity hPi0 h1; repeat move/[apply].
    move => [d][h0]h1.
    exists d.
  ...
  - move => Γ A A' i B M M' hA ihA hB hM ihM T U.
    ...
    move : OExps.ηcommutativity hLam'. repeat move/[apply].
    move => [Q][hQ0]hQ1.
  ...
  - move => Γ A i B B' M M' N N' hA hB ihB hM ihM hN ihN u T.
    ...
    move : OExps.ηcommutativity hApp1. repeat move/[apply].
    move => [d][hh0]hh1.
    exists d. split => //=.
...
Lemma ηregularity Γ a b A : Γ ⊢ a ▻η b ∈ A -> Γ ⊢ a ∈ A /\ Γ ⊢ b ∈ A.
Proof. qauto l:on use:WtExp_embed, wr_lh_refl, wr_rh_refl. Qed.
...
Proof.
  move => Γ M N A + h.
  elim : Γ M N A / h.
  - hauto lq:on use:β_lh_refl,wr_rh_refl, WtExp_embed.
  - hauto lq:on use:β_lh_refl,wr_rh_refl, WtExp_embed.
  - move => Γ i A A' B B' hA ihA hB ihB P.
    move /factorization => [U][hP0]hP1.
    move /IInv.Prod_inv : hP0.
    move => [A'0][B'0][i0][?][h0][h1]h2. subst.
    apply Univ_Inj in h2. subst.
    move : (h0) => {}/ihA . move => [A''][ihA0]ihA1.
    move : h1 => {}/ihB. move => [B''][ihB0]ihB1.
    have : Γ ⊢ Pi A'0 B'0 ▻β Pi A'' B'' ∈ Univ i.
    constructor. by eauto.
    by apply /βCtx_step; eauto using WtExp_embed.
    move : OExps.βcommutativity hP1. repeat move/[apply].
    move => [d]h. decompose [ and ] h => {h}.
    (* Need βO_commute *)
    exists d. split => //. apply : merge'; eauto with eexp.
    econstructor; eauto. apply /ηCtx_step; eauto using WtBRed_embed.
  - move => Γ A A' i B M M' hA ihA hB hM ihM U /factorization.
    move => [V][hV0]hV1.
    move /IInv.Lam_inv : hV0.
    move => [A'0][M'0][B0][i0][?][h0][h1][h2]h3. subst.
    have [? ?] : Γ ⊢ A ∈ Univ i /\ Γ ⊢ A ∈ Univ i0 by hauto l:on use:ηregularity, βregularity.
    have ? : i0 = i by sfirstorder use:unique_sorts_mutual. subst.
    move : (h0) => {}/ihA .  move => [A''][ihA0]ihA1.
    have {h2} : A :: Γ ⊢ M ▻η M'0 ∈ B by
      apply : ηexchange'';eauto; hauto lq:on use:wr_lh_refl, WtBRed_embed.
    move  => {}/ihM.
    move => [M''][ihM0]ihM1.
    have : Γ ⊢ Lam A'0 M'0 ▻β Lam A'' M'' ∈ Pi A B.
    apply : WB_Conv. apply : WB_Lam; eauto. apply : Ctx_step. apply hB. by eauto using WtExp_embed.
    apply : βCtx_step. by eauto. by eauto using WtExp_embed.
    apply : WE_Exp; by econstructor; eauto using WtExp_embed.
    move : OExps.βcommutativity hV1. repeat move/[apply].
    move => [d [? ?]].
    exists d. split => //.
    apply : merge'; eauto. apply WE_Conv with (A := Pi A' B). apply : WE_Lam; eauto.
    apply : Ctx_step; by eauto using WtBRed_embed.
    apply : ηCtx_step; by eauto using WtBRed_embed.
    apply : WE_Exp. constructor; eauto using WtBRed_embed.
  - move => Γ A i B B' M M' N N' hA hB ihB hM ihM hN ihN U /factorization.
    move => [V][hV0]hV1.
    move /IInv.App_inv : hV0.
    move => [A''][B''][N''][i'][hA'][hB'][hN'][e][M''][hM']?. subst.
    have e0 : Γ ⊢ A'' ≡ A by eauto with unique.
    have ? : i' = i by sfirstorder use:equiv_sort_unique. subst.
    have {}hB' : A :: Γ ⊢ B ▻η B'' ∈ Univ i by eauto with conv.
    move : (hB') => {}/ihB. move => [B'0][ihB0]ihB1.
    have {}hN' : Γ ⊢ N ▻η N'' ∈ A by eauto with conv.
    move : (hN') => {}/ihN. move => [N'0][ihN0]ihN1.
    have ePi : Γ ⊢ Pi A'' B ≡ Pi A B by eauto with unique.
    have {}hM' : Γ ⊢ M ▻η M'' ∈ Pi A B by eauto with conv.
    move : (hM') => {}/ihM. move => [M'0][ihM0]ihM1.
    have : Γ ⊢ (App B'' M'' N'') ▻β (App B'0 M'0 N'0) ∈ B[N..].
    apply WB_Conv with (A := B''[N''..]).
    apply : WB_App. apply hA'.  by eauto with conv.
    suff : Γ ⊢ Pi A B ≡ Pi A'' B'' by eauto with conv.
    apply : WE_Trans; eauto using Equiv_sym.
    apply : WE_Red.
    constructor; by eauto using WtExp_embed with conv.
    by eauto with conv.
    apply : WE_Exp. apply WR_cong_univ with (A := A); eauto with conv unique.
    move : OExps.βcommutativity hV1; repeat move/[apply].
    move => [d][? ?].
    exists d. split => //.
    apply : merge'; eauto.
