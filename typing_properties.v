...
Proof.
  move => + h. elim : h; eauto with wt.
Qed.

Lemma WRs_Conv Γ a b A B : Γ ⊢ a ▻+ b ∈ A -> Γ ⊢ A ≡ B -> Γ ⊢ a ▻+ b ∈ B.
Proof.
  elim; qauto l:on use:WR_Conv db:wt.
Qed.
...
Proof.
  elim : h; hauto q:on ctrs:WtEquiv use:wt_renaming_mutual.
Qed.
...
Lemma wrs_Equiv Γ i A B (h : Γ ⊢ A ▻+ B ∈ Univ i) : Γ ⊢ A ≡ B.
Proof.
  move E : (Univ i) h => T h.
  move : E.
  elim : A B T / h;
...
Proof. move =>>. elim; eauto. Qed.

#[export]Hint Resolve WtRed_Wf : wt.

Lemma Wf_cons_inv A Γ :
  ⊢ A :: Γ ->
  ⊢ Γ /\ exists B i, Γ ⊢ A ▻ B ∈ Univ i.
Proof. hauto l:on inv:Wf use:WtRed_Wf. Qed.
...
Lemma WRs_Ctx_conv A B Γ M N C (h : A :: Γ ⊢ M ▻+ N ∈ C) (h1 : Γ ⊢ A ≡ B)
  ...
  B :: Γ ⊢ M ▻+ N ∈ C.
Proof.
  elim : h; qauto l:on use:Ctx_conv db:wt.
Qed.
...
Proof.
  ...
  move : i E.
  elim : h.
  - move =>  M N A1 hM i ? hB. subst.
    have ? : ⊢ M :: Γ by eauto with wt.
    apply WRs_One.
    apply : WR_Prod=>//.
    apply : Ctx_conv; eauto with wt.
  - move => M N A1 P h0 h1 ih i ? h2. subst.
...
Proof.
  ...
  - move => Γ A i A' B B' M M' N N' hA ihA hB ihB hM ihM hN ihN ρ0 ρ1 Δ hρ hΔ /=.
    eapply WR_Beta' with (M' := M'[up_tm_tm ρ1]) (N' := N'[ρ1]) (i := i) ; eauto using good_morphing2_up ; cycle 1.
    + by asimpl.
    + hauto lq:on rew:off use:good_morphing2_up,lookup_good_morphing2_lh_refl  db:wt.
    + hauto lq:on rew:off use:good_morphing2_up,lookup_good_morphing2_lh_refl  db:wt.
    + by asimpl.
...
Proof.
  move => + h.
  move /wt_morphing2_mutual.
  apply; last by sfirstorder use:WtRed_Wf.
  move => k A0.
  elim /lookup_inv=>_.
  - move => ? ? ? []*. subst. by asimpl.
  - move => n A1 Γ0 B0 ? ? []*. subst.
    asimpl. hauto lq:on  db:wt.
Qed.
...
Proof.
  ...
  - move => Γ A A' i B B' M M' hA ihA hB ihB hM ihM.
  ...
  - move => Γ A A' i B B' M M' N N' hA ihA hB ihB hM ihM hN ihN.
    ...
    apply : WE_Exp.
    apply /WR_cong : hB hN.
  - move => Γ A i A'  B B' M M' N N' hA ihA hB _ hM ihM hN ihN.
    apply : WR_Conv.
    apply : WR_cong; eauto.
    apply : WE_Exp.
    move /lh_refl_mutual : hB hN.
    move /WR_cong. move/[apply]. simpl. apply.
Qed.
...
Proof.
  ...
  - move => A Γ /Wf_cons_inv.
    move => [h0][B][i]h1.
    exists i.
    move /lh_refl_mutual : (h1).
    move/wt_renaming_mutual.
...
Lemma wr_lh_refl :
  (forall Γ a b A, Γ ⊢ a ▻ b ∈ A -> Γ ⊢ a ▻ a ∈ A ).
Proof. exact lh_refl_mutual. Qed.

Lemma wrs_lh_refl :
  (forall Γ a b A, Γ ⊢ a ▻+ b ∈ A -> Γ ⊢ a ▻ a ∈ A ).
Proof. induction 1; sfirstorder use:wr_lh_refl. Qed.

Lemma wr_rh_refl :
  (forall Γ a b A, Γ ⊢ a ▻ b ∈ A -> Γ ⊢ b ▻ b ∈ A ).
Proof. exact rh_refl_mutual. Qed.

Lemma wrs_rh_refl :
  (forall Γ a b A, Γ ⊢ a ▻+ b ∈ A -> Γ ⊢ b ▻ b ∈ A ).
Proof. induction 1; sfirstorder use:wr_rh_refl. Qed.
...
Inductive WtEquivS Γ M N i : Prop :=
| WES_Red :
  Γ ⊢ M ▻ N ∈ Univ i ->
  (* ------------------- *)
  Γ ⊢ M ≃ N ∈ i
...
| WES_Trans P :
  ...
  Γ ⊢ M ≃ N ∈ i
where
"Γ ⊢ A ≃ B ∈ i" := (WtEquivS Γ A B i).

#[export]Hint Constructors WtEquivS : wt.
...
Lemma WtEquiv_regularity Γ A B (h : Γ ⊢ A ≡ B) : exists i, Γ ⊢ B ▻ B ∈ Univ i.
Proof. induction h; eauto with wt. Qed.

Lemma WtEquivS_regularity Γ A B i (h : Γ ⊢ A ≃ B ∈ i) : Γ ⊢ B ▻ B ∈ Univ i.
Proof. induction h; eauto with wt. Qed.

Lemma Ctx_conv' A B Γ M N C (h : A :: Γ ⊢ M ▻ N ∈ C) (h1 : Γ ⊢ A ≡ B) :
  B :: Γ ⊢ M ▻ N ∈ C.
Proof. hauto lq:on use:Ctx_conv, WtEquiv_regularity db:wt. Qed.

#[export]Hint Resolve Ctx_step : wt.


Lemma WtEquivS_embed_WtEquiv Γ M N i : Γ ⊢ M ≃ N ∈ i -> Γ ⊢ M ≡ N.
Proof. induction 1; eauto with wt. Qed.

Lemma WtEquivS_sym Γ M N i : Γ ⊢ M ≃ N ∈ i -> Γ ⊢ N ≃ M ∈ i.
Proof. induction 1; eauto with wt. Qed.

Lemma Pi_functionality0 Γ A B0 B1 i :
  Γ ⊢ A ▻ A ∈ Univ i ->
  A::Γ ⊢ B0 ≃ B1 ∈ i ->
  Γ ⊢ Pi A B0 ≃ Pi A B1 ∈ i.
Proof.
  move => h0 h1.
  move : B0 B1 i h1 h0.
  apply : WtEquivS_ind; eauto with wt.
Qed.

Lemma Pi_functionality1 Γ A0 A1 B i :
  Γ ⊢ A0 ≃ A1 ∈ i ->
  A0::Γ ⊢ B ▻ B ∈ Univ i ->
  Γ ⊢ Pi A0 B ≃ Pi A1 B ∈ i.
Proof.
  move : A0 A1 i.
  apply : WtEquivS_ind; eauto with wt.
  - hauto lq:on use:Ctx_conv', Equiv_sym db:wt.
  - move => A0 A2 i A1 hA0 ihA0 hA1 ihA1 hB.
    move /(_ hB) in ihA0.
    have : A1 :: Γ ⊢ B ▻ B ∈ Univ i by qauto l:on use:WtEquivS_embed_WtEquiv, Ctx_conv' db:wt.
    move /ihA1.
    eauto with wt.
Qed.

Lemma Pi_functionality Γ i A0 A1 B0 B1 :
  Γ ⊢ A0 ≃ A1 ∈ i ->
  A0 :: Γ ⊢ B0 ≃ B1 ∈ i ->
  Γ ⊢ Pi A0 B0 ≃ Pi A1 B1 ∈ i.
Proof.
  move => h0 h1.
  apply : WES_Trans.
  apply Pi_functionality0; eauto using WtEquivS_regularity with wt.
  by move /WtEquivS_sym /WtEquivS_regularity : h0.
  apply Pi_functionality1=>//.
  by move  /WtEquivS_regularity : h1.
Qed.
...
Lemma Univ_inv Γ i N T (h : Γ ⊢ Univ i ▻ N ∈ T) :
  (N = Univ i \/ N = TmUnit /\ Γ ⊢ T ≡ TyUnit /\ Γ ⊢ TyUnit ≡ Univ (S i)) /\ Γ ⊢ T ≡ Univ (S i).
Proof.
  move E : (Univ i) h => M h.
  move : i E.
...
Lemma Var_inv Γ n N T (h : Γ ⊢ var_tm n ▻ N ∈ T) :
  exists A, (N = var_tm n \/ N = TmUnit /\ Γ ⊢ T ≡ TyUnit /\ Γ ⊢ TyUnit ≡ A) /\ lookup n Γ A /\ Γ ⊢ T ≡ A.
Proof.
  ...
  - hauto q:on db:wt.
  - (* move => Γ M N A B i hM ihM hA _ n ?. subst. *)
    (* specialize ihM with (1 := eq_refl). *)
    (* move : ihM => [A0][i0][h0][h1]h2. *)
    (* exists A0, i0. repeat split =>//. *)
    hauto q:on db:wt.
  - hauto q:on db:wt.
Qed.

Lemma Prod_inv Γ A B N T (h : Γ ⊢ Pi A B ▻ N ∈ T) :
  exists A' B' i, (N = Pi A' B' \/ N = TmUnit /\ Γ ⊢ T ≡ TyUnit /\ Γ ⊢ TyUnit ≡ Univ i) /\ Γ ⊢ A ▻ A' ∈ Univ i /\ A::Γ ⊢ B ▻ B' ∈ Univ i /\ Γ ⊢ T ≡ Univ i.
Proof.
  move E : (Pi A B) h => M h.
  move : A B E.
  elim : Γ M N T / h=>//; hauto lq:on db:wt.
Qed.

Lemma Lam_inv Γ A M N T (h : Γ ⊢ Lam A M ▻ N ∈ T) :
  exists A' M' B i,
    (N = Lam A' M' \/ N = TmUnit /\ Γ ⊢ T ≡ TyUnit /\ Γ ⊢ TyUnit ≡ Pi A B) /\
    Γ ⊢ A ▻ A' ∈ Univ i /\
    A::Γ ⊢ B ▻ B ∈ Univ i /\
    A::Γ ⊢ M ▻ M' ∈ B /\
    Γ ⊢ T ≡ Pi A B.
Proof.
  ...
  move : A M E.
  elim : Γ M0 N T / h=>//; hauto q:on db:wt.
Qed.
...
Lemma App_inv Γ P B Q N T (h : Γ ⊢ App B P Q ▻ N ∈ T) :
  exists A A' B' Q' i,
    ...
    Γ ⊢ T ≡ B[Q..] /\
    (* App case *)
    ((exists P', Γ ⊢ P ▻ P' ∈ Pi A B /\ N = App B' P' Q') \/
    (* Beta case *)
     (exists R R', P = Lam A R /\ A::Γ ⊢ R ▻ R' ∈ B /\ N = R'[Q'..]) \/
    (* UnitEta case *)
    (N = TmUnit /\ Γ ⊢ T ≡ TyUnit /\ Γ ⊢ TyUnit ≡ B[Q..])).
Proof.
  ...
  - move => Γ A A' i B B' M M' N N' hA _ hB _ hM _ hN _ >[]*. subst.
    ...
    + apply : WE_Red.
      ...
      apply /lh_refl_mutual.
    + sfirstorder.
  - move => Γ A i A'  B B' M M' N N' hA _ hB _ hM _ hN _ > []*.
    subst. exists A, A', B, N', i.
    repeat split => //; first by eauto with wt.
    (* Factor out the first bullet *)
    + apply : WE_Red.
      move /WR_cong: hB hN. repeat move/[apply].
      apply /lh_refl_mutual.
...
Lemma TyUnit_inv Γ N T (h : Γ ⊢ TyUnit ▻ N ∈ T) :
  Γ ⊢ T ≡ Univ 0 /\ (N = TyUnit \/ N = TmUnit /\ Γ ⊢ T ≡ TyUnit /\ Γ ⊢ TyUnit ≡ Univ 0).
Proof.
  move E : TyUnit h => M h.
  move : E.
  elim : Γ M N T / h=>//; hauto q:on db:wt.
Qed.

Lemma TmUnit_inv Γ N T (h : Γ ⊢ TmUnit ▻ N ∈ T) :
  Γ ⊢ T ≡ TyUnit /\ (N = TyUnit \/ N = TmUnit).
Proof.
  move E : TmUnit h => M h.
  move : E.
  elim : Γ M N T / h=>//; hauto q:on db:wt.
...
Lemma Wt_unique_mutual :
  (forall Γ a b A, Γ ⊢ a ▻ b ∈ A -> forall c B, Γ ⊢ a ▻ c ∈ B -> Γ ⊢ A ≡ B).
Proof.
  apply : WtRed_ind; eauto with wt.
  - qauto l:on use:Var_inv, lookup_deter db:wt.
  - hauto lq:on use:Univ_inv db:wt.
  - hauto lq:on use:Prod_inv db:wt.
  - move => Γ A A' i B B' M M' hA ihA hB ihB hM ihM ? T /Lam_inv.
    move =>[A'0][M'0][B0][i0][h0][h2][h3][h4]h5.
    apply Equiv_sym.
    apply : WE_Trans; eauto.
    move /ihA in h2.
    move /ihM in h4.

  - move => Γ A A' i B M M' hA ihA hB ihB hC ihC.
    move => ? T /Lam_inv.
    move => [A'0][M'0][B0][i0][h0][h1][h2][h3]h4.
    case : h0.
    + move => ?. subst.
      have {}/ihA := h1 => ihA.

      move /ihA in h1.
      move  /WE_Trans : h4. apply.



(* Lemma unit_eq Γ a b c A B (h : Γ ⊢ a ▻ b ∈ A) : *)
(*   Γ ⊢ A ≡ TyUnit -> *)
(*   Γ ⊢ a ▻ c ∈ B -> Γ ⊢ B ≡ TyUnit. *)
(* Proof. *)
(*   move : c B. *)
(*   elim : Γ a b A / h=>//. *)
(*   - move => Γ n A hΓ hA ? c A0 /Var_inv. subst. *)
(*     move => [A1][hA0][]. *)
(*     + move => [?]h. subst. *)
(*       have ? : A = A1 by eauto using lookup_deter. subst. *)
(*       eauto with wt. *)
(*     + move => [?][h]h1. done. *)
(*   - move => Γ i hΓ c B heq. *)
(*     move /Univ_inv. *)
(*     qauto l:on db:wt. *)
(*   - move => Γ i A A' B B' hA ihA hB ihB c B0 heq. *)
(*     move /Prod_inv. *)
(*     qauto l:on db:wt. *)
(*   - move => Γ A A' i B M M' hA ihA hB ihB hM ihM c T heq /Lam_inv. *)
(*     move => [A'0][M'0][B1][i0][h][h1][h2][h3]h4. *)
(*     case : h. *)
(*     + move => ?. subst. *)
(*       best. *)
(*       suff : Γ ⊢ Pi A B ≡ Pi A B1 by hauto lq:on use:Equiv_sym db:wt. *)
(*       apply *)
(*   - move => Γ A A' i B B' M M' N N' hA _ hB _ hM _ hN _ h. *)
(*     move => P T /App_inv. *)
(*     sfirstorder db:wt. *)



(* Lemma unit_eq Γ a b c A (h : Γ ⊢ a ▻ b ∈ TyUnit) : *)
(*   Γ ⊢ a ▻ c ∈ A -> Γ ⊢ A ≡ TyUnit. *)
(* Proof. *)
...
(*     move => [A][hA0][]. *)
(*     + move => [?]h. subst. *)
(*       have ? : A = TyUnit by eauto using lookup_deter. *)
(*       congruence. *)
(*     + move => [?][h]h1. by subst. *)
(*   - move => Γ A A' i B B' M M' N N' hA _ hB _ hM _ hN _ h. *)
(*     move => P T /App_inv. *)
(*     sfirstorder db:wt. *)
(*   - move => Γ A i A' A0 B M M' N N' hA _ hA' _ hA00 _ hA01 _ hM _ hN _ h. *)
(*     move => P T /App_inv. *)
(*     sfirstorder db:wt. *)
(*   - move => Γ hΓ _ c A /TmUnit_inv. *)
(*     sfirstorder. *)
(*   - move => Γ M N A B i hM ihM hA ihA ?. subst. *)
(*     move => N0 T hT. *)
...
Proof.
  ...
  9 : {
    move => Γ a b ha ih c A.
    move /ih => h0 .
    split=>//.
    have : Γ ⊢ a ▻ TmUnit ∈ TyUnit.

}
  - move => Γ n A hΓ hn P B /Var_inv.
    + move => [A0][h].
      case.
      * qauto l:on use:WR_Conv' db:wt.
      * move => [?][h0]h1. subst.
        split.
        apply : WR_Conv'; eauto with wt.
        have ? : A = A0 by eauto using lookup_deter. subst.
        apply : WR_Conv; eauto.
        apply : WR_TmUnitEta.
        apply : WR_Conv'; eauto.
        apply WR_Var=>//.
  - hauto lq:on rew:off use:WR_Conv' use:Univ_inv db:wt.
  - move => Γ i A A' B B' hA ihA hB ihB P B0.
    move /Prod_inv.
    move => [A0][B1][i0][_][hA0][hB0]hB1.
    apply : WR_Conv'; eauto.
    apply WR_Prod; eauto.
  - move => Γ A A' i B M M' hA ihA hB ihB hM ihM P B0 /Lam_inv.
    move => [A'0][M'0][B1][i0][_][h0][h1][h2]h3.
    apply : WR_Conv'; eauto.    (* don't merge the two eautos *)
    eauto with wt.
...
(*     of the App cases *) *)
    ...
    apply : WR_Conv'; eauto.
    hauto lq:on db:wt.
  - hauto lq:on rew:off use:TyUnit_inv, WR_Conv' db:wt.
  - hauto lq:on use:TmUnit_inv, WR_Conv' db:wt.
  - move => Γ a b ha ih c B.
    move => h.
    have hu : Γ ⊢ a ▻ TmUnit ∈ TyUnit by eauto with wt.

    move /ih in h.
    move /wr_rh_refl in h.
    move /wr_rh_refl in ha.
...
Proof.
  ...
  8 : {
    move => Γ hΓ P B hP.
    have h : Γ ⊢ TmUnit ▻ TmUnit ∈ TyUnit by auto with wt.
    exists TmUnit.
    split.
    + eauto using exchange.
    + have : Γ ⊢ P ▻ P ∈ TyUnit by eauto using exchange with wt.
      eauto with wt.
  }
  (* TmUnitEta *)
  8 : {
    move => Γ a b ha iha P B ha0.
    exists TmUnit.
    split; eauto using exchange with wt.
  }

  - move => Γ n A hΓ hn ? B /Var_inv.
    move => [A0][h0][[? h1]|[? [h1 h2]]]; subst.
    + hauto l:on use:WR_Conv' db:wt.
    + have ? : A = A0 by eauto using lookup_deter. subst.
      exists TmUnit. split; eauto using WR_Conv, WR_Conv' with wt.
